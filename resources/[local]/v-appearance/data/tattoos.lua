-- v-appearance | tattoo catalogue (GTA V ped decoration overlays)
-- Factual game data: each entry is a Rockstar tattoo overlay (collection + the
-- gender-specific overlay name) grouped by body zone. Applied via
-- AddPedDecorationFromHashes(ped, joaat(collection), joaat(hash<M|F>)).
-- There is no native that enumerates these at runtime, so the list is shipped.
