...
def check_callbacks(report):
    ...
    answered = set()
    for path in (_glob('client', '*.lua') +
                 _glob('bridge', 'client', '*.lua') +
                 _glob('apps', '**', '*.lua', recursive=True)):
...
def check_keys(report):
    ...
    sources = (_glob('server', '*.lua') +
               _glob('client', '*.lua') +
               _glob('bridge', '**', '*.lua', recursive=True) +
               _glob('apps', '**', '*.lua', recursive=True) +
