...
-- Detected by v-core's module registry, which lists it in the admin panel under this
-- label. The settings themselves are declared from the server, through V.Module.
v_module 'yes'
...
shared_scripts {
    ...
    'bridge/shared/v.lua',
    -- v-core's own V, layered over the standalone one directly above. The bridge file
    -- reimplements this framework's library for a server that does not run it; here the
    -- framework IS running, so the real one wins. Both open with `V = V or {}`, which is
    -- what makes this a layer rather than a replacement: the twenty-nine functions they
    -- share come from v-core, and the six the bridge adds on top of them survive.
    '@v-core/lib/v.lua',
