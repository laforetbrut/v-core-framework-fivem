...
- **v-clothing module** — a proper clothing store (Ponsonbys / Suburban / Binco peds + blips) with **live on-ped preview**, **auto-detected variations** (only valid drawables/textures via the ped natives), a front camera you rotate by dragging the mouse, and prices per category. Bought pieces become **inventory items**; **equip** by using the item, **unequip** from the wardrobe tab (the piece returns to your inventory). Worn pieces persist in the character appearance. Fully fr/en, CEF-103-safe.
