...
- **Clothing catalogue thumbnails (admin scan)** — an in-game thumbnail generator: `screenshot-basic` (Cfx.re, self-building at boot) captures every clothing drawable on the admin's ped (clean scene, per-category skeleton-framed camera) and stores the image, so the catalogue tiles show a **real photo** instead of a number. Triggered by the admin **F9 keybind** (press twice to confirm; rebindable in GTA key settings — this server has no chat), scanning only the missing pieces; the `/scanclothes [all|new|<category>]` command remains for chat-enabled setups. Permission-gated, server-side writes guarded to the scanning admin, thumbnails lazy-loaded per tile in the NUI (falls back to the number when a piece hasn't been scanned yet).
...
- **Miniatures du catalogue de vêtements (scan admin)** — un générateur de vignettes in-game : `screenshot-basic` (Cfx.re, auto-compilé au démarrage) capture chaque vêtement sur le ped de l'admin (scène propre, caméra cadrée par os du squelette selon la catégorie) et stocke l'image, pour que les cases du catalogue affichent une **vraie photo** au lieu d'un numéro. Déclenché par le **raccourci admin F9** (double appui pour confirmer ; réassignable dans les paramètres clavier GTA — ce serveur n'a pas de chat), qui scanne uniquement les pièces manquantes ; la commande `/scanclothes [all|new|<catégorie>]` reste disponible pour les serveurs avec chat. Protégé par permission, écritures serveur restreintes à l'admin en cours de scan, vignettes chargées à la volée par case dans le NUI (repli sur le numéro tant qu'une pièce n'est pas scannée).
