...
- **Clothing catalogue thumbnails (admin scan)** — an in-game thumbnail generator: `screenshot-basic` (Cfx.re, self-building at boot) captures every clothing drawable on the admin's ped (clean scene, per-category skeleton-framed camera) and stores the image, so the catalogue tiles show a **real photo** instead of a number. Admin command **`/scanclothes`** (all), **`/scanclothes new`** (only newly-added pieces), or **`/scanclothes <category>`** — permission-gated, server-side writes guarded to the scanning admin, thumbnails lazy-loaded per tile in the NUI (falls back to the number when a piece hasn't been scanned yet).
...
- **Miniatures du catalogue de vêtements (scan admin)** — un générateur de vignettes in-game : `screenshot-basic` (Cfx.re, auto-compilé au démarrage) capture chaque vêtement sur le ped de l'admin (scène propre, caméra cadrée par os du squelette selon la catégorie) et stocke l'image, pour que les cases du catalogue affichent une **vraie photo** au lieu d'un numéro. Commande admin **`/scanclothes`** (tout), **`/scanclothes new`** (uniquement les nouveautés) ou **`/scanclothes <catégorie>`** — protégée par permission, écritures serveur restreintes à l'admin en cours de scan, vignettes chargées à la volée par case dans le NUI (repli sur le numéro tant qu'une pièce n'est pas scannée).
